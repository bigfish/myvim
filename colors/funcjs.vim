...
if exists('g:funcjs_colors')
	...
	let s:fun_colors = [ 252, 2, 11, 172, 1, 161, 63 ]
endif
...
function! FunScope()
	...
	for fn in s:js_functions
		...
		if fn.start[0] == fn.block_end[0]
			call matchadd('F' . (fn.depth), '\%' . fn.start[0] . 'l\%>' . (fn.start[1] - 1) . 'c.*\%<' . (fn.block_end[1] + 1) . 'c' , fn.depth) 

		elseif (fn.start[0] + 1) == fn.block_end[0]
			"two line functions
			call matchadd('F' . (fn.depth), '\%' . fn.start[0] . 'l\%>' . (fn.start[1] - 1) . 'c.*', fn.depth) 
			call matchadd('F' . (fn.depth), '\%' . fn.block_end[0] . 'l.*\%<' . (fn.block_end[1] + 1) . 'c' , fn.depth) 
		else
			"multiline functions
			call matchadd('F' . (fn.depth), '\%' . fn.start[0] . 'l\%>' . (fn.start[1] - 1) . 'c.*', fn.depth) 
			call matchadd('F' . (fn.depth), '\%>' . fn.start[0] . 'l.*\%<' . fn.block_end[0] . 'l', fn.depth) 
			call matchadd('F' . (fn.depth), '\%' . fn.block_end[0] . 'l.*\%<' . (fn.block_end[1] + 1) . 'c' , fn.depth) 
		endif
	endfor
