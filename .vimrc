...
function! ParseFunction(start_pos, depth, stopline)
	...
	if search('\<function\>', 'W', a:stopline) > 0
		...
		echom "function starts at " . start_function_line . ', ' . start_function_col
		...
		"this isn't working -- changing cursor pos
		...
		while !empty(inner_func) 
			...
			"allow only within function body
			"if inner_func.block_end[0] < end_brace_pos[0] ||
			"\(inner_func.block_end[0] == end_brace_pos[0] && 
			"\inner_func.block_end[1] < end_brace_pos[1])
...
function! FunScope()
	...
	let fun_colors = [ 6, 5, 4, 3, 2, 1 ]

	let c = 1
	for colr in fun_colors
	...
	while !empty(func)
		...
		"if len(func.inner_functions)
			"let js_functions += func.inner_functions
		"endif
	...
	for fn in s:js_functions
		...
		if fn.start[0] == fn.block_end[0]
			call matchadd('F' . (fn.depth + 1), '\%' . fn.start[0] . 'l\%>' . (fn.start[1] - 1) . 'c.*\%<' . (fn.block_end[1] + 1) . 'c' , fn.depth) 

		elseif (fn.start[0] + 1) == fn.block_end[0]
			"two line functions
			call matchadd('F' . (fn.depth + 1), '\%' . fn.start[0] . 'l\%>' . (fn.start[1] - 1) . 'c.*', fn.depth) 
			call matchadd('F' . (fn.depth + 1), '\%' . fn.block_end[0] . 'l.*\%<' . (fn.block_end[1] + 1) . 'c' , fn.depth) 
		else
			"multiline functions
			call matchadd('F' . (fn.depth + 1), '\%' . fn.start[0] . 'l\%>' . (fn.start[1] - 1) . 'c.*', fn.depth) 
			call matchadd('F' . (fn.depth + 1), '\%>' . fn.start[0] . 'l.*\%<' . fn.block_end[0] . 'l', fn.depth) 
			call matchadd('F' . (fn.depth + 1), '\%' . fn.block_end[0] . 'l.*\%<' . (fn.block_end[1] + 1) . 'c' , fn.depth) 
		endif
	endfor
